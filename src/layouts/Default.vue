  ...
  <q-layout view="lHh Lpr lFf">
    <q-layout-header class="head">
      <q-toolbar class="headbar flex transparent">
        ...
        <q-btn flat dense round @click="leftopen" aria-label="Menu" class="menu icon icon-menu"/>
      </q-toolbar>
    </q-layout-header>
    ...
    >
      <div class="bigin flex">
        <button
          @click="login"
          v-ripple
          class="bigining icon capitalize icon-dengluyonghuming glossy"
          v-if="islogin"
        >{{thislang.login}}</button>
      ...
      <div class="statistical smalltop flex">
        <q-knob v-model="infos.cpu" :min="0" :max="100" size="1.52rem" class="knob" readonly>
          <p class="knobp">{{infos.cpu}}%</p>
          <p class="knobp">CPU</p>
        </q-knob>
        <q-knob v-model="infos.net" :min="0" :max="100" size="1.52rem" class="knob" readonly>
          <p class="knobp">{{infos.net}}%</p>
          <p class="knobp">NET</p>
        </q-knob>
      </div>
      <ul class="tolanguage smalltop flex">
        <div class="langitem" :class="changeI == 1?'active':''" @click="changeL(1)">
          <img src="../common/images/icon9.png">
        </div>
        <div class="langitem" :class="changeI == 0?'active':''" @click="changeL(0)">
          <img src="../common/images/icon1.png">
        </div>
        <div class="langitem" :class="changeI == 2?'active':''" @click="changeL(2)">
          <img src="../common/images/icon9.png">
        </div>
      </ul>
...
export default {
  ...
  computed: {
    ...mapGetters(["thislang","infos", "packages"]),
    // vuex中侧滑状态
    myopenleft() {
  ...
  methods: {
    ...
    getbaoinfo(data) {
      get("/get_money_list", data).then(val => {
        ...
        console.log(newdata[this.packages.this])
    ...
    getinfos() {
      ...
      return new Promise(function(resolve, reject) {
        get("/get_info").then(val => {
					let {in_packet_count, in_packet_sum, out_packet_count, transaction_info_count, user_count, xinyunjiangchi} = val.data || {}
          let info = {
            in_packet_count,
            in_packet_sum,
            out_packet_count,
